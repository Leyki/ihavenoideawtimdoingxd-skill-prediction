...
function* flatten(iterable) {
	for (const value of iterable) {
		if (isIterable(value)) yield* flatten(iterable);
		else yield value;
	}
}

function _search(parents, children, path, child, results, keyToSearch) {
	for (const [k, v] of Object.entries(child)) {
		...
		if (typeof v === "object") {
			if (v != null && !parents.has(v)) {
				parents.add(v);
				try {
					_search(parents, children, path, v, results, keyToSearch);
				} catch (err) { /* empty */ }
			}
			if (keyToSearch === k)
...
class Utilities {
	...
	static getDataFromObjectByField(root, keyToSearch) {
		...
		if (typeof root === "object" && root != null)
			_search(parents, children, path, root, results, keyToSearch);
		return results;
	}
