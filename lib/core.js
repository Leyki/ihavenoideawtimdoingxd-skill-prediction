...
module.exports = function SPCore(mod) {
	const skillsClient = require('../config/skills_client' + (mod.majorPatchVersion < 80 ? '' : '_80'))
...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
			...
			if(clientAction.skill.type === TYPE_ACTION) {
				...
				reaction: {
					if(clientAction.air) {
						...
						if(((clientAction.skill.id === player.templateId*100 + 2   && !player.templateId === 10911  )|| (player.templateId === 10911 && clientAction.skill.id === 1101102 )) && info.dcType === 'stand' ) break reaction
	...
	mod.hook('S_EACH_SKILL_RESULT', 13, event => {
		...
		if(isMe(event.target) && reaction.enable) {
			if(mod.settings.debug.skills) {
				const duration = Date.now() - debugActionTime,
					strs = ['<- S_EACH_SKILL_RESULT.reaction', skillId(reaction.skill), reaction.stage, '[' + Number(reaction.push), Number(reaction.air), Number(reaction.airChain) + ']']
