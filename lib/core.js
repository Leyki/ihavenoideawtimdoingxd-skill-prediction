...
module.exports = function SkillPrediction(dispatch) {
	...
	function enable() {
		...
		try {
			addHook("S_DEFEND_SUCCESS", 3, sDefendSuccessHandler);
			addHook("S_CANNOT_START_SKILL", 2, sCannotStartSkillHandler);
			addHook("C_CAN_LOCKON_TARGET", 3, cCanLockonTargetHandler);
			addHook("S_CAN_LOCKON_TARGET", 3, sCanLockonTarget);
			addHook("C_PLAYER_LOCATION", 5, cPlayerLocationHandler);
			addHook("C_NOTIFY_LOCATION_IN_ACTION", 4, notifyLocation.bind(null, "C_NOTIFY_LOCATION_IN_ACTION", 4));
			addHook("C_NOTIFY_LOCATION_IN_DASH", 4, notifyLocation.bind(null, "C_NOTIFY_LOCATION_IN_DASH", 4));
			for (let packet of [
				["C_START_SKILL", 7],
			...
			addHook("S_GRANT_SKILL", 3, sGrantSkillHandler);
			addHook("S_INSTANT_DASH", 3, sInstantDashHandler);
			addHook("S_INSTANT_MOVE", 3, sInstantMoveHandler);
			addHook("S_ACTION_END", 5, sActionEndHandler);
			addHook("C_CANCEL_SKILL", 3, cCancelSkillHandler);
			addHook("S_ACTION_STAGE", 8, sActionStageHandler);
			addHook("S_CREST_MESSAGE", 2, state.hookFake, sCrestMessageHandler);
			addHook("S_START_COOLTIME_SKILL", 3, cooldownFix);
			addHook("S_DECREASE_COOLTIME_SKILL", 3, cooldownFix);
		...
		if (isMe(event.gameId)) {
			if (state.config.debug) {
				let duration = Date.now() - debugActionTime,
					strs = [skillInfo(event.skill) ? "<X" : "<-", "S_ACTION_STAGE", skillId(event.skill), event.stage, `${utils.decimal(event.speed, 3)}x`, `${utils.decimal(event.projectileSpeed, 3)} x`];
	...
	dispatch.hook("S_EACH_SKILL_RESULT", 11, event => {
	...
	function sCannotStartSkillHandler(event) {
		...
		if (skillInfo(event.skill)) {
			if (state.config.skillDelayOnFail && state.config.skillRetryCount && currentAction && (!serverAction || !currentAction.skill.equals(serverAction.skill)) && event.skill === currentAction.skill.id)
	...
	function cCanLockonTargetHandler(event) {
		...
		if (info) {
			...
			dispatch.toClient("S_CAN_LOCKON_TARGET", 3, Object.assign({
				success
			}, event));
		}
	}
	...
	function sendActionStage(opts) {
		...
		dispatch.toClient("S_ACTION_STAGE", 8, currentAction = {
			gameId: myChar(),
			loc: currentLocation.loc,
			w: currentLocation.w,
			templateId,
	...
	function sendCannotStartSkill(skill) {
		dispatch.toClient("S_CANNOT_START_SKILL", 2, {skill: skill.id});
	}
