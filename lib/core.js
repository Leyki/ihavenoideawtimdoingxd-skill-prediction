...
module.exports = function SkillPrediction(dispatch) {
    ...
    function startSkill(type, version, code, data) {
        ...
        if(info.type === "charging" && Date.now() - buyingPingRemoverWhenTho <= (config.skillRetryCount * config.skillRetryMs)) delay += (config.skillRetryCount * config.skillRetryMs);
    ...
    function sendActionEnd(type, distance) {
        ...
        buyingPingRemoverWhenTho = Date.now();
